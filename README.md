...
| 🌲 **树形可视化编辑** | 拖拽排序、重命名、新建子文件夹与书签 |
| 📊 **变更检测与选择** | 自动识别新增/删除的书签，用户可选择保留或确认变更后再写回 |
| 📈 **实时进度反馈** | 同步、AI 整理、写回操作均显示进度条与百分比 |
| 📋 **统计信息面板** | 实时显示总书签数、文件夹数、不同域名数、上次同步时间 |
...
| **应用回浏览器** | 将侧栏中的修改写回 Chrome，自动检测并同步新增/删除的书签 |
| **⚙ 设置** | 配置 AI 服务商、API Key、模型等 |

### 变更检测与写回

应用回浏览器时，TabMark 会自动对比当前书签与同步时的基准：

| 场景 | 处理方式 |
|------|------|
| 新增书签 | 列出详情，确认后写入浏览器 |
| 删除书签 | 列出详情，可选择「全部保留」或确认删除 |
| 数量一致 | 直接写回标题、位置、分类变更 |

- 每次操作均显示**实时进度条**与百分比
- 统计面板实时展示书签数、文件夹数、域名数
...
├── sidepanel.html         # 侧栏页面（进度条、统计卡片、变更面板）
├── sidepanel.css          # 侧栏样式（深色主题）
├── sidepanel.js           # 侧栏主逻辑（树 UI、事件、进度、统计）
├── lib/
│   ├── bookmarks.js       # Chrome bookmarks API 同步 / 写回（支持进度回调）
│   ├── integrity.js       # 书签变更检测（新增/删除识别）
